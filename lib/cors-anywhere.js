...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if(location && location.pathname) {
      var clientID, apiSecret, devBoxList, stgBoxList, siteOrigin;
      clientID = "";
      apiSecret = "";
      devBoxList = ['http://localhost:4200', 'http://aia-joinrenew-dev.s3-website-us-east-1.amazonaws.com', 'https://aia-joinrenew-dev.s3-website-us-east-1.amazonaws.com'];
      stgBoxList = ['https://aia-joinrenew-stg.s3-website-us-east-1.amazonaws.com', 'http://aia-joinrenew-stg.s3-website-us-east-1.amazonaws.com'];
      siteOrigin = (req.headers && req.headers.origin) ? req.headers.origin : "";
      if(devBoxList.indexOf(siteOrigin) !== -1) {
        clientID = "jhxbmd679pv6j3hatb3yz7xdpgtauced";
        apiSecret = "mx3zd27mcvdbkrmnr829cczcvnnr62dq";
      } else if(stgBoxList.indexOf(siteOrigin) !== -1) {
        clientID = "jhxbmd679pv6j3hatb3yz7xdpgtauced";
        apiSecret = "mx3zd27mcvdbkrmnr829cczcvnnr62dq";
      }
      if(requestDataEndPoints.indexOf(location.pathname) !== -1 || authHeaderEndPoints.indexOf(location.pathname) !== -1) {      
        if(authHeaderEndPoints.indexOf(location.pathname) !== -1) {
          req.headers.Authorization = 'Basic '+ (base64.encode(clientID+":"+apiSecret));
        }
        if(requestDataEndPoints.indexOf(location.pathname) !== -1) {
          req.url = addParameterToURL("client_id="+clientID+"&client_secret="+apiSecret, req.url);        
        }
      }
    }
    var location = parseURL(req.url.slice(1));
